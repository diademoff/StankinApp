...
| GET   | `/api/schedule/by-subject` | Расписание по предмету, преподавателю и группе (весь семестр) |
...
**Параметры `/api/schedule/by-subject` (Предмет + преподаватель + группа):**

| Параметр | Тип | Пример |
|----------|-----|--------|
| `subject` | string | `Дискретная математика` |
| `teacher` | string | `Козлов А.Б.` |
| `groupName` | string | `АДБ-24-03` |
